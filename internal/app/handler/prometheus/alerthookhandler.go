...
// @accept  json
// @produce json
// @param data body PrometheusAlert true "Alert Model"
...
func doHandle(model PrometheusAlert) {
...
func doHandleEvalMatch(model AlertDetails) {
	...
	namespace, podName, containerName := labels.Namespace, labels.Pod, labels.Container
	...
	if err != nil {
		log.Printf("crawlString labels: %v, error: %v\n", labels, err)
		return
	}
...
func storeStack(stack string, model AlertDetails) error {
	labels := model.Labels
	namespace, podName, containerName, node := labels.Namespace, labels.Pod, labels.Container, labels.Node
